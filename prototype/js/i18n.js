/** @typedef {'en' | 'ja'} Locale */

export const LOCALE_STORAGE_KEY = 'aetheria-locale';
/** Default locale when nothing is saved (user designs in Japanese). */
export const DEFAULT_LOCALE = 'ja';

/** @type {Locale} */
let locale = loadLocale();

/** @type {Set<(loc: Locale) => void>} */
const listeners = new Set();

function loadLocale() {
  try {
    const saved = localStorage.getItem(LOCALE_STORAGE_KEY);
    if (saved === 'en' || saved === 'ja') return saved;
  } catch {
    /* localStorage unavailable */
  }
  return DEFAULT_LOCALE;
}

if (typeof document !== 'undefined') {
  document.documentElement.lang = locale === 'ja' ? 'ja' : 'en';
}

/** @returns {Locale} */
export function getLocale() {
  return locale;
}

/** @param {Locale} newLocale */
export function setLocale(newLocale) {
  if (newLocale !== 'en' && newLocale !== 'ja') return;
  if (newLocale === locale) return;
  locale = newLocale;
  try {
    localStorage.setItem(LOCALE_STORAGE_KEY, newLocale);
  } catch {
    /* ignore */
  }
  if (typeof document !== 'undefined') {
    document.documentElement.lang = newLocale === 'ja' ? 'ja' : 'en';
  }
  for (const fn of listeners) fn(locale);
}

/** @param {(loc: Locale) => void} fn */
export function onLocaleChange(fn) {
  listeners.add(fn);
  return () => listeners.delete(fn);
}

/**
 * @param {string} key dot-separated path
 * @param {Record<string, string | number>} [params]
 */
export function t(key, params = {}) {
  const parts = key.split('.');
  let node = STRINGS[locale];
  for (const p of parts) {
    node = node?.[p];
  }
  if (typeof node !== 'string') {
    node = STRINGS.en;
    for (const p of parts) {
      node = node?.[p];
    }
  }
  let text = typeof node === 'string' ? node : key;
  for (const [k, v] of Object.entries(params)) {
    text = text.replaceAll(`{${k}}`, String(v));
  }
  return text;
}

/** Cost list separator for build labels. */
export function costSeparator() {
  return locale === 'ja' ? '、' : ', ';
}

const STRINGS = {
  en: {
    subtitle: '2D Browser Prototype — Floating Continent at 50 km',
    settings: {
      title: 'Settings',
      language: 'Language',
      close: 'Close',
      ariaGear: 'Open settings',
    },
    newGame: {
      title: 'New Game',
      intro: 'Choose difficulty. Earth periodic aid (H₂O + Fe) varies by difficulty and is locked for the run.',
      difficulty: 'Difficulty',
      easy: 'Easy',
      normal: 'Normal',
      hard: 'Hard',
      easyDesc: 'Aid: +4 H₂O, +2 Fe every 120 ticks · CORE solar + electrolysis boosted',
      normalDesc: 'Aid: +2 H₂O, +1 Fe every 120 ticks · standard CORE bootstrap',
      hardDesc: 'Aid: none · lean on Earth market (Fe/H₂O) and sulfur export (2 S → 6₵)',
      hardMarketHint: 'Hard: no periodic aid — use Inventory for Fe/H₂O buys and sulfur export for ₵',
      start: 'Start',
    },
    panel: {
      continentStatus: 'Floating Continent Status',
      buoyancy: 'Buoyancy',
      mass: 'Mass',
      netLift: 'Net Lift',
      power: 'Power',
      windLoad: 'Wind Load',
      corrosion: 'Corrosion',
      resources: 'Resources',
      sulfur: 'Sulfur',
      iron: 'Iron',
      earthCredits: 'Earth Credits',
      buildModule: 'Build Module',
      buildHint: 'Click an empty unit adjacent to your floating continent.',
      buildHintActive: 'Click an empty adjacent hex to place. Esc or Cancel to exit build mode.',
      buildHintNeedFeForSolar: 'Buy one more Fe → Solar Array',
      buildHintSolarFirst: 'Select Solar Array → click adjacent hex',
      buildHintIsruNext: 'Next: ISRU Refinery (watch power)',
      buildHintShortage: 'Some modules need materials — open Inventory or wait for aid.',
      powerDeficitSolar: 'Power deficit — add Solar',
      buildHintH2CellLift: 'Negative lift — build H₂ Buoyancy Cell',
      buildHintH2Extend: 'Select H₂ cell → Extend',
      buildHintElectrolyzer: 'H₂ lift failing — build Water Electrolyzer',
      buildHintElectrolyzerOptional: 'Tip: Water Electrolyzer converts H₂O → H₂',
      h2LiftCoreElectro: 'CORE electrolysis active — keep H₂O stocked',
      h2LiftCritical: 'H₂ lift gas low — electrolysis, ISRU acid split, or coat against leak',
      h2VentEmergency: 'Emergency: vent cargo in Inventory to shed mass',
      h2LiftStatus: 'H₂ lift +{lift}t · gas {effective}/{demand}t · leak −{leak}/tick',
      structuralLift: 'Structural lift',
      h2GasLift: 'H₂ gas lift',
      exportSulfurProgress: '{have}/{need}t',
      buildModeBanner: 'Building: {module} — click adjacent hex',
      cancelBuild: 'Cancel build',
      selectedUnit: 'Selected Unit',
      selectedNone: 'Click a module to select.',
      extendH2: 'Extend H₂ Layer (+H₂ lift, +wind)',
      extendH2Short: 'Extend',
      lowerH2: 'Lower H₂ Layer (−buoyancy, −wind)',
      lowerH2Short: 'Lower',
      applyCoating: 'Sulfur Coating (1t S, preventive / −corrosion)',
      applyCoatingShort: 'Coat',
      carbonLighten: 'Carbon Composite (−1t C: mass↓, lift↑)',
      carbonLightenShort: 'Lighten',
      dismantleModule: 'Dismantle Module (partial Fe refund)',
      dismantleModuleShort: 'Scrap',
      coatingHint: 'High corrosion — apply sulfur coating (S) to slow damage',
      coatingDisabledNoSulfur: 'Need {amount}t S for coating',
      coatingJumpWorst: 'Jump to worst unit ({max}%)',
      tradeEarth: 'Trade with Earth (S → Credits)',
      inventory: 'Inventory & Earth Market (export)',
      buildPowerGen: '+{gen} power',
      buildPowerUse: '−{use} power',
      buildPowerNetAfter: 'net after {net}',
      buildMassDelta: 'Δmass {delta}t',
      buildNetLiftDelta: 'Δlift {delta}t',
      buildAcidEtaAfter: 'H₂SO₄ ETA ~{eta} ticks',
      buildAcidEtaSaved: 'Δacid −{delta} ticks',
      acidEtaHud: '~{eta} ticks to next 1t (intake ×{units})',
      liftRunway: 'Lift runway',
      liftRunwayTicks: '~{ticks} ticks',
      liftRunwayNow: 'negative now',
      liftRunwayStable: 'stable',
      earthContract: 'Earth micro-contract',
      buildMaterialShort: 'Need: {detail}',
      sulfurUpkeep: 'S upkeep −{amount}/tick (corrosion control)',
      corrosionWorst: 'worst {max}% · {count} penalized',
      sulfurExportHint: '→ Export S in Inventory',
      creditsExportHint: 'Earn via sulfur export',
      tickRate: '1 tick / second',
      difficulty: 'Difficulty',
      isruStatus: 'ISRU Status',
      isruDetails: 'ISRU & details',
      statusMore: 'More metrics',
      otherResources: 'Other resources',
      earthAid: 'Earth Periodic Aid',
      earthAidEtaShort: 'Aid in {eta} ticks (+{h2o} H₂O, +{iron} Fe)',
      earthAidNone: 'No aid',
      sulfurExportWait: 'Sulfur short for export — wait for ISRU acid split',
      buyIron: 'Buy Fe ({price}₵)',
      buyH2o: 'Buy H₂O ({price}₵)',
      exportSulfurShort: 'Export S ({cost}t→₵)',
      buildShortMaterial: '{material} short',
      buildShortPower: 'power {net}',
      o2Flow: 'Electrolysis +{produce} / life support −{consume} per tick',
      coreBootstrap: 'CORE: +{power} solar, +{h2} H₂/tick electrolysis (needs H₂O)',
    },
    confirm: {
      title: 'Confirm',
      ok: 'Confirm',
      cancel: 'Cancel',
    },
    footer: 'CORE intakes CO₂ and runs small solar + water electrolysis (H₂O→H₂). ISRU: acid split → H₂, Bosch → C. Vent any resource in Inventory (except credits) in emergencies.',
    inventory: {
      title: 'Inventory & Earth Market',
      intro: 'Buy Fe/H₂O, export sulfur for credits (time paused while open).',
      pauseNotice: 'Time does not advance while this dialog is open (ticks paused).',
      earthTradeIntro: 'Export Venus products to Earth for credits (no unit selection needed).',
      holdings: 'All holdings',
      buy: 'Buy ({price}₵)',
      exportSulfur: 'Export to Earth ({cost}t S → credits)',
      ventCargo: 'Vent to atmosphere (−{amount}t)',
      ventCargoHint: 'Reduces carried mass (§7.1)',
      ventEmergencyHint: 'Emergency jettison — resource is lost (no mass effect for gases)',
      creditsHint: 'Earn by exporting sulfur to Earth',
      close: 'Close',
    },
    gameover: {
      title: 'Continent Sank',
      body: 'Buoyancy failed — the floating continent slipped beneath the sulfur clouds of Venus.',
      hope: 'This is not the end — revise your design and try again.',
      survivalTicks: 'Survival ticks:',
      restart: 'Restart',
      cause: {
        h2Lift: 'Primary cause: effective H₂ gas fell far below lift demand.',
        mass: 'Primary cause: total mass was too high — net lift collapsed.',
        power: 'Primary cause: power shortage kept ISRU and electrolysis idle.',
        corrosion: 'Primary cause: heavy corrosion drove H₂ leak faster than you could recover.',
      },
      causeSecondary: {
        h2Lift: 'H₂ lift gas was also short of demand.',
        mass: 'Carried and structural mass weighed the continent down.',
        power: 'Negative power net stalled ISRU processing.',
        corrosion: 'Corrosion and leak made H₂ upkeep harder.',
      },
    },
    sink: {
      warning: '⚠ Low buoyancy — sinking in {remaining} ticks',
      caution: 'Negative lift — caution ({remaining} ticks to sink)',
      warningH2: '⚠ H₂ lift collapse — sinking in {remaining} ticks',
      cautionH2: 'H₂ lift gas short — caution ({remaining} ticks to sink)',
    },
    tick: 'Tick {n}',
    paused: 'Paused',
    pause: 'Pause',
    resume: 'Resume',
    selected: '{name} at ({coords})\nFloor area: {floorArea}\nCorrosion: {corrosion}%',
    selectedCorrosionEffects: 'Corrosion effects: power +{power}, mass +{mass}, lift −{lift}',
    selectedH2Cell: '{name} at ({coords})\nFloor area: {floorArea}\nH₂ layers: {layers} | Corrosion: {corrosion}%',
    module: {
      core: 'Core Habitat',
      intake: 'Atmospheric Intake',
      isru: 'ISRU Refinery',
      solar: 'Solar Array',
      h2cell: 'H₂ Buoyancy Cell',
      electrolyzer: 'Water Electrolyzer',
    },
    moduleShort: {
      intake: 'Intake',
      isru: 'ISRU',
      solar: 'Solar',
      h2cell: 'H₂ Cell',
      electrolyzer: 'Electrolyzer',
    },
    difficulty: {
      easy: 'Easy',
      normal: 'Normal',
      hard: 'Hard',
    },
    material: {
      co2: { name: 'Carbon dioxide', desc: 'Dominant atmospheric gas. Harvested by CORE/intake.', obtain: 'Atmospheric intake (CORE)' },
      carbon: { name: 'Carbon', desc: 'Structural carbon from CO₂ reduction (Bosch).', obtain: 'ISRU Bosch' },
      n2: { name: 'Nitrogen', desc: 'Secondary atmospheric component.', obtain: 'Atmospheric intake (CORE)' },
      h2so4: { name: 'Sulfuric acid', desc: 'Trace harvest from acid clouds. ISRU feedstock.', obtain: 'Atmospheric intake (CORE)' },
      h2: { name: 'Hydrogen', desc: 'Essential lift gas. Leaks faster as corrosion worsens. From acid split, water electrolysis, CORE bootstrap.', obtain: 'ISRU acid split / electrolysis / CORE' },
      sulfur: { name: 'Sulfur', desc: 'Coatings and module construction. Exportable to Earth.', obtain: 'ISRU acid split' },
      o2: { name: 'Oxygen', desc: 'Byproduct of CO₂ electrolysis; consumed by CORE life support each tick.', obtain: 'ISRU electrolysis (net ≈0 with CORE)' },
      h2o: { name: 'Water', desc: 'Scarce. Periodic Earth aid + market + ISRU recovery.', obtain: 'Earth aid / market / ISRU' },
      iron: { name: 'Iron', desc: 'Required for all module frames. Earth aid + market.', obtain: 'Earth aid / market' },
      credits: { name: 'Earth credits', desc: 'Paid Earth market currency. Earned by exporting sulfur.', obtain: 'Earth trade' },
    },
    unit: { t: 't', cr: 'cr', floorArea: '~100 m²' },
    msg: {
      gameOver: 'Game over. Please restart.',
      unknownMaterial: 'Unknown material.',
      materialLocked: '{name} is not available yet.',
      materialNotBuyable: '{name} cannot be purchased.',
      invalidAmount: 'Invalid quantity.',
      insufficientCredits: 'Insufficient credits (need {need}₵, have {have}₵). Export sulfur to Earth to earn more.',
      bought: 'Bought {name} ×{amount} for {cost}₵.',
      mustBeAdjacent: 'Must build on a unit adjacent to the floating continent.',
      noBuildSelected: 'Select a module type to build first.',
      invalidModule: 'Invalid module.',
      insufficientMaterials: 'Insufficient materials: {detail}. Buy from the Earth market in Inventory, or wait for intake.',
      missingEntry: '{name} (need {need}, have {have})',
      built: 'Built {name} (spent {cost})',
      noModuleSelected: 'No module selected.',
      needH2Extend: 'Need {amount}t hydrogen to extend H₂ layer.',
      h2CellOnly: 'H₂ layer extend/lower is only available on H₂ buoyancy cells.',
      h2MaxHeight: 'H₂ layer is at maximum height.',
      h2Extended: 'H₂ envelope extended — H₂ lift up, wind load up.',
      h2MinHeight: 'H₂ layer is already at minimum height.',
      h2Lowered: 'H₂ envelope lowered — buoyancy down, wind load down.',
      needSulfurCoating: 'Need {amount}t sulfur for acid-resistant coating.',
      coatingApplied: 'Sulfur coating applied — protection active, rise slowed.',
      needSulfurTrade: 'Need {amount}t sulfur to trade with Earth.',
      traded: 'Exported sulfur to Earth — +{gain}₵ credits',
      restarted: 'Restarted with a new floating continent.',
      isruAcidSplit: 'ISRU: acid split ×{amount} — H₂ + S',
      isruBosch: 'ISRU: Bosch ×{amount} — C produced',
      isruWaitingAcid: 'ISRU waiting — need H₂SO₄ from intake',
      isruWaitingH2: 'ISRU waiting — H₂ short for Bosch',
      isruH2Reserve: 'ISRU: Bosch skipped — H₂ held in reserve for buoyancy',
      powerDeficit: 'Power deficit — ISRU idle',
      windShear: 'Wind shear warning — reinforce or reduce H₂ height',
      windDamage: 'High wind load — extra corrosion and power drain',
      needCarbonLighten: 'Need {amount}t carbon for composite lightening.',
      carbonLightenMax: 'Carbon lightening is at maximum for this module.',
      carbonLightened: 'Carbon composite applied — mass down, buoyancy up.',
      sinkStart: '⚠ Low buoyancy — the continent is sinking!',
      sinkStartH2: '⚠ H₂ lift gas depleted — the continent is sinking!',
      sinkCountdown: 'Sinking in {remaining} ticks',
      sank: 'The continent sank beneath the clouds…',
      buoyancyRecovered: 'Buoyancy recovered — sink countdown cleared',
      earthAid: 'Earth periodic aid arrived — +{h2o} H₂O, +{iron} Fe',
      cannotDismantleCore: 'CORE cannot be dismantled.',
      cannotDismantleLast: 'Cannot dismantle the last module.',
      cannotDismantleBridge: 'Cannot dismantle — would disconnect the floating continent.',
      confirmDismantle: 'Dismantle {name}? +{iron} Fe will be returned. This cannot be undone.',
      dismantled: 'Dismantled {name} — mass reduced.',
      cannotVentMaterial: '{name} cannot be vented.',
      insufficientCargo: 'Not enough {name} (need {need}, have {have}).',
      confirmVent: 'Vent {amount}t of {name} into the atmosphere? Resources will be lost.',
      ventedCargo: 'Vented {amount}t {name} — carried mass reduced.',
      corrosionWarn: 'Acid corrosion rising — sulfur coating slows damage (select unit → Coating).',
      corrosionSevere: 'Severe corrosion — power drain and lift loss worsening. Coat modules or spend S on upkeep.',
      corrosionCritical: 'Critical corrosion — efficiency heavily reduced. Apply sulfur coating now.',
      sUpkeepActive: 'Sulfur upkeep active — {amount}/tick spent slowing corrosion on {count} modules.',
      h2LeakWarn: 'Hydrogen leaking — {rate}t/tick (corrosion-driven; coating slows leak)',
      electrolyzerRun: 'Electrolyzer: water split ×{amount} — H₂ produced',
      contractComplete: 'Earth contract complete — {title}: {reward}',
      buildLiftWarn: 'Warning: net lift after build would be {lift}t — still placing',
    },
    contract: {
      rewardH2o: '+{amount} H₂O',
      rewardFe: '+{amount} Fe',
      rewardCredits: '+{amount}₵',
      fourModules: {
        title: 'Expand the continent',
        progress: '+{current}/{target} modules since contract ({total} total goal)',
      },
      holdLift: {
        title: 'Hold steady lift',
        progress: '{current}/{target} ticks at net lift ≥ +{minLift} ({remaining} left)',
      },
      acidSplits: {
        title: 'Acid split runs',
        progress: '{current}/{target} sulfuric acid splits',
      },
    },
    isru: {
      status: {
        noIsru: 'No ISRU module',
        noPower: 'Power deficit — idle',
        acidReady: 'Acid split ready',
        boschReady: 'Bosch ready',
        waitingAcid: 'Waiting on H₂SO₄ intake',
        waitingH2: 'H₂ short for Bosch',
        electrolyzing: 'Bosch blocked — CO₂ electrolysis',
        idle: 'Idle',
      },
      acidProgress: 'Next H₂SO₄ batch: {pct}% (~{eta} ticks)',
      acidProgressNoEta: 'Next H₂SO₄ batch: {pct}%',
      intakeHintAffordable: 'Tip: build an Atmospheric Intake module to speed acid harvest',
      intakeHintFeShort: 'Iron short — wait for Earth periodic aid or buy from Earth market',
      intakeHintFeShortWithAid: 'Iron short — next Earth aid in {eta} ticks, or buy from market',
      intakeHintCarbonShort: 'Carbon short — produce via ISRU Bosch',
      intakeHintSulfurShort: 'Sulfur short — produce via ISRU acid split',
      fallbackElectrolysis: 'Fallback: CO₂ electrolysis continues meanwhile',
    },
  },
  ja: {
    subtitle: '2Dブラウザ試作版 — 高度50kmの浮遊大陸',
    settings: {
      title: '設定',
      language: '言語',
      close: '閉じる',
      ariaGear: '設定を開く',
    },
    newGame: {
      title: 'ニューゲーム',
      intro: '難易度を選んでください。地球定期援助（H₂O + Fe）は難易度で決まり、プレイ中は変更できません。',
      difficulty: '難易度',
      easy: 'やさしい',
      normal: 'ふつう',
      hard: 'むずかしい',
      easyDesc: '援助：120ティックごとに H₂O +4、Fe +2 · コアのソーラー・電解強化',
      normalDesc: '援助：120ティックごとに H₂O +2、Fe +1 · 標準コア初期装備',
      hardDesc: '援助：なし · 地球市場（Fe/H₂O）と硫黄輸出（2 S → 6₵）を活用',
      hardMarketHint: 'むずかしい：定期援助なし — 在庫の地球市場で Fe/H₂O を購入、硫黄輸出で ₵ を得る',
      start: '開始',
    },
    panel: {
      continentStatus: '浮遊大陸ステータス',
      buoyancy: '浮力',
      mass: '質量',
      netLift: '正味浮力',
      power: '電力',
      windLoad: '風荷重',
      corrosion: '腐食',
      resources: '資源',
      sulfur: '硫黄',
      iron: '鉄',
      earthCredits: '地球クレジット',
      buildModule: 'モジュール建設',
      buildHint: '浮遊大陸に隣接する空きユニットをクリック。',
      buildHintActive: '隣接する空きヘックスをクリックして配置。Esc またはキャンセルで建設モード終了。',
      buildHintNeedFeForSolar: 'Feをもう1つ→ソーラー',
      buildHintSolarFirst: 'ソーラー→隣接',
      buildHintIsruNext: '次はISRU（電力注意）',
      buildHintShortage: '材料不足のモジュールあり — 在庫または援助を確認。',
      powerDeficitSolar: '電力不足 — ソーラー追加',
      buildHintH2CellLift: '正味浮力↓ — H₂セルを建設',
      buildHintH2Extend: 'H₂セルを選択→延伸',
      buildHintElectrolyzer: 'H₂浮力不足 — 水電解モジュールを建設',
      buildHintElectrolyzerOptional: 'ヒント: 水電解モジュールで H₂O→H₂',
      h2LiftCoreElectro: 'コア内蔵電解稼働中 — H₂O を確保',
      h2LiftCritical: 'H₂昇力ガス不足 — 電解・硫酸分解・漏れ抑制（コーティング）',
      h2VentEmergency: '緊急: 在庫から貨物を排出して質量を削減',
      h2LiftStatus: 'H₂昇力 +{lift}t · ガス {effective}/{demand}t · 漏れ −{leak}/tick',
      structuralLift: '構造浮力',
      h2GasLift: 'H₂昇力',
      exportSulfurProgress: '{have}/{need}t',
      buildModeBanner: '建設中: {module} — 隣接ヘックスをクリック',
      cancelBuild: '建設をキャンセル',
      selectedUnit: '選択中のユニット',
      selectedNone: 'モジュールをクリックして選択。',
      extendH2: 'H₂層を延伸（H₂昇力↑・風荷重↑）',
      extendH2Short: '延伸',
      lowerH2: 'H₂層を下降（浮力↓・風荷重↓）',
      lowerH2Short: '下降',
      applyCoating: '硫黄コーティング（S 1t・予防／腐食↓）',
      applyCoatingShort: 'コーティング',
      carbonLighten: '炭素複合材（C 1t：質量↓・浮力↑）',
      carbonLightenShort: '軽量化',
      dismantleModule: 'モジュール解体（Fe 一部返却）',
      dismantleModuleShort: '解体',
      coatingHint: '腐食が進行中 — 硫黄コーティング（S）で被害を抑えられます',
      coatingDisabledNoSulfur: 'コーティングに S {amount}t が必要',
      coatingJumpWorst: '最大腐食ユニットへ（{max}%）',
      tradeEarth: '地球へ輸出（S → クレジット）',
      inventory: '在庫・地球市場（輸出）',
      buildPowerGen: '+{gen} 電力',
      buildPowerUse: '−{use} 電力',
      buildPowerNetAfter: '配置後 {net}',
      buildMassDelta: 'Δ質量 {delta}t',
      buildNetLiftDelta: 'Δ正味浮力 {delta}t',
      buildAcidEtaAfter: 'H₂SO₄ 到達 ~{eta} ティック',
      buildAcidEtaSaved: 'Δ酸待ち −{delta} ティック',
      acidEtaHud: '次の 1t まで約 {eta} ティック（採取ユニット ×{units}）',
      liftRunway: '浮力余裕',
      liftRunwayTicks: '約 {ticks} ティック',
      liftRunwayNow: 'すでにマイナス',
      liftRunwayStable: '安定',
      earthContract: '地球ミニ契約',
      buildMaterialShort: '不足: {detail}',
      sulfurUpkeep: 'S維持 −{amount}/tick（腐食抑制）',
      corrosionWorst: '最大 {max}%・ペナルティ {count} 基',
      sulfurExportHint: '→ 在庫で地球へ輸出',
      creditsExportHint: '硫黄の輸出で獲得',
      tickRate: '1秒 = 1ティック',
      difficulty: '難易度',
      isruStatus: 'ISRU状態',
      isruDetails: 'ISRU・詳細',
      statusMore: '詳細メトリクス',
      otherResources: 'その他の資源',
      earthAid: '地球定期援助',
      earthAidEtaShort: '次の援助まで {eta} ティック（+{h2o} H₂O、+{iron} Fe）',
      earthAidNone: '援助なし',
      sulfurExportWait: '硫黄不足 — ISRU硫酸分解を待つ',
      buyIron: 'Feを買う（{price}₵）',
      buyH2o: 'H₂Oを買う（{price}₵）',
      exportSulfurShort: 'S輸出（{cost}t→₵）',
      buildShortMaterial: '{material}不足',
      buildShortPower: '電力{net}',
      o2Flow: '電解 +{produce}／生命維持 −{consume}（毎ティック）',
      coreBootstrap: 'コア: ソーラー +{power}、電解 +{h2} H₂/tick（H₂O必要）',
    },
    confirm: {
      title: '確認',
      ok: '実行',
      cancel: 'キャンセル',
    },
    footer: 'コアはCO₂採取と小型ソーラー・内蔵水電解（H₂O→H₂）を常時実行。ISRU：硫酸分解→H₂、ボッシュ→C。緊急時は在庫からクレジット以外を排出可能。',
    inventory: {
      title: '在庫・地球市場',
      intro: 'Fe/H₂O を購入・硫黄を輸出してクレジット獲得（開いている間は時間停止）。',
      pauseNotice: '開いている間は時間が進みません（ティック停止）。',
      earthTradeIntro: '金星産物を地球へ輸出してクレジットを獲得できます（ユニット選択不要）。',
      holdings: '全在庫',
      buy: '購入（{price}₵）',
      exportSulfur: '地球へ輸出（{cost}t S → クレジット）',
      ventCargo: '大気へ排出（−{amount}t）',
      ventCargoHint: '搭載質量を削減（§7.1）',
      ventEmergencyHint: '緊急投棄 — 資源は失われる（気体は質量に影響なし）',
      creditsHint: '硫黄を地球へ輸出して獲得',
      close: '閉じる',
    },
    gameover: {
      title: '浮遊大陸が沈没しました',
      body: '浮力が持たず、浮遊大陸は金星の硫酸雲の下へ消えていきました。',
      hope: 'まだ終わりではありません — 設計を見直して、もう一度挑戦しましょう。',
      survivalTicks: '生存ティック:',
      restart: '再起動',
      cause: {
        h2Lift: '主因: 有効 H₂ ガスが昇力需要を大きく下回った。',
        mass: '主因: 総質量が高く、正味浮力が大きく落ちた。',
        power: '主因: 電力不足で ISRU・電解が止まり続けた。',
        corrosion: '主因: 腐食と H₂ 漏れが回復を上回った。',
      },
      causeSecondary: {
        h2Lift: 'H₂ 昇力ガスも需要に対して不足していた。',
        mass: '搭載・構造質量が大陸を押し下げていた。',
        power: '電力ネットがマイナスで ISRU が停止していた。',
        corrosion: '腐食と漏れで H₂ 維持が難しかった。',
      },
    },
    sink: {
      warning: '⚠ 浮力不足 — 沈没まで {remaining} ティック',
      caution: '浮力がマイナス — 注意（{remaining} ティックで沈没）',
      warningH2: '⚠ H₂昇力崩壊 — 沈没まで {remaining} ティック',
      cautionH2: 'H₂昇力ガス不足 — 注意（{remaining} ティックで沈没）',
    },
    tick: 'ティック {n}',
    paused: '一時停止中',
    pause: '一時停止',
    resume: '再開',
    selected: '{name}（{coords}）\n床面積: {floorArea}\n腐食: {corrosion}%',
    selectedCorrosionEffects: '腐食影響: 電力 +{power}・質量 +{mass}・浮力 −{lift}',
    selectedH2Cell: '{name}（{coords}）\n床面積: {floorArea}\nH₂層: {layers} | 腐食: {corrosion}%',
    module: {
      core: 'コア居住域',
      intake: '大気インテーク',
      isru: 'ISRU精製所',
      solar: 'ソーラーアレイ',
      h2cell: 'H₂浮力セル',
      electrolyzer: '水電解モジュール',
    },
    moduleShort: {
      intake: 'インテーク',
      isru: 'ISRU',
      solar: 'ソーラー',
      h2cell: 'H₂セル',
      electrolyzer: '水電解',
    },
    difficulty: {
      easy: 'やさしい',
      normal: 'ふつう',
      hard: 'むずかしい',
    },
    material: {
      co2: { name: '二酸化炭素', desc: '大気の主成分。コア／インテークで常時採取。', obtain: '大気採取（コア）' },
      carbon: { name: '炭素', desc: 'CO₂還元（ボッシュ）で得る構造用炭素。', obtain: 'ISRUボッシュ' },
      n2: { name: '窒素', desc: '大気の二次成分。', obtain: '大気採取（コア）' },
      h2so4: { name: '硫酸', desc: '硫酸雲からの微量回収。ISRUの原料。', obtain: '大気採取（コア）' },
      h2: { name: '水素', desc: '浮力維持に必須。腐食が進むと漏れが加速。硫酸分解・水電解・コア内蔵電解で供給。', obtain: 'ISRU硫酸分解／電解／コア' },
      sulfur: { name: '硫黄', desc: '防食コーティングやモジュール建設に使用。地球へ輸出可。', obtain: 'ISRU硫酸分解' },
      o2: { name: '酸素', desc: 'CO₂電解の副産物。コアの生命維持で毎ティック消費。', obtain: 'ISRU電解（コア稼働時は正味≈0）' },
      h2o: { name: '水', desc: '希少資源。地球定期援助・市場・ISRU回収。', obtain: '地球援助／市場／ISRU' },
      iron: { name: '鉄', desc: '全モジュール建設に必須。地球援助・市場。', obtain: '地球援助／市場' },
      credits: { name: '地球クレジット', desc: '地球市場での購入通貨。硫黄の輸出で獲得。', obtain: '地球取引' },
    },
    unit: { t: 't', cr: 'cr', floorArea: '約100 m²' },
    msg: {
      gameOver: 'ゲームオーバーです。再起動してください。',
      unknownMaterial: '不明な材料です。',
      materialLocked: '{name}はまだ利用できません。',
      materialNotBuyable: '{name}は購入できません。',
      invalidAmount: '数量が不正です。',
      insufficientCredits: 'クレジット不足（必要 {need}₵、所持 {have}₵）。硫黄を地球へ輸出して稼いでください。',
      bought: '{name} ×{amount} を {cost}₵ で購入しました。',
      mustBeAdjacent: '浮遊大陸に隣接するユニットにのみ建設できます。',
      noBuildSelected: '先に建設するモジュール種別を選んでください。',
      invalidModule: '無効なモジュールです。',
      insufficientMaterials: '材料不足：{detail}。在庫画面で地球市場から購入するか、採取を待ってください。',
      missingEntry: '{name}（必要 {need}、所持 {have}）',
      built: '{name} を建設しました（{cost} 消費）',
      noModuleSelected: 'モジュールが選択されていません。',
      needH2Extend: 'H₂層の延伸には {amount}t の水素が必要です。',
      h2CellOnly: 'H₂層の延伸・下降は H₂浮力セルのみ可能です。',
      h2MaxHeight: 'H₂層は最大高度に達しています。',
      h2Extended: 'H₂エンベロープを延伸 — H₂昇力↑、風荷重↑',
      h2MinHeight: 'H₂層はすでに最低高度です。',
      h2Lowered: 'H₂エンベロープを下降 — 浮力↓、風荷重↓',
      needSulfurCoating: '耐酸コーティングには硫黄 {amount}t が必要です。',
      coatingApplied: '硫黄コーティングを適用 — 防食効果が有効、上昇速度が緩和されます。',
      needSulfurTrade: '地球への輸出には硫黄 {amount}t が必要です。',
      traded: '地球へ硫黄を輸出 — +{gain}₵ クレジット',
      restarted: '新しい浮遊大陸で再開しました。',
      isruAcidSplit: 'ISRU：硫酸分解 ×{amount} — H₂・S 生成',
      isruBosch: 'ISRU：ボッシュ ×{amount} — C 生成',
      isruWaitingAcid: 'ISRU待機中 — インテークの H₂SO₄ を待っています',
      isruWaitingH2: 'ISRU待機中 — ボッシュ用 H₂ が不足',
      isruH2Reserve: 'ISRU：ボッシュ保留 — 浮力用 H₂ を温存中',
      powerDeficit: '電力不足 — ISRU停止中',
      windShear: '風切り警告 — 補強するかH₂高度を下げてください',
      windDamage: '風荷重過大 — 腐食上昇・電力消費増',
      needCarbonLighten: '軽量化には炭素 {amount}t が必要です。',
      carbonLightenMax: 'このモジュールの炭素軽量化は上限に達しています。',
      carbonLightened: '炭素複合材を適用 — 質量↓、浮力↑',
      sinkStart: '⚠ 浮力不足 — 浮遊大陸が沈み始めています！',
      sinkStartH2: '⚠ H₂昇力ガス不足 — 浮遊大陸が沈み始めています！',
      sinkCountdown: '沈没まであと {remaining} ティック',
      sank: '浮遊大陸が雲の下へ沈没しました…',
      buoyancyRecovered: '浮力回復 — 沈没カウントダウン解除',
      earthAid: '地球定期援助到着 — H₂O +{h2o}、Fe +{iron}',
      cannotDismantleCore: 'コア（CORE）は解体できません。',
      cannotDismantleLast: '最後のモジュールは解体できません。',
      cannotDismantleBridge: '解体できません — 浮遊大陸の接続が切れます。',
      confirmDismantle: '{name} を解体しますか？鉄 {iron}t が返却されます。元に戻せません。',
      dismantled: '{name} を解体 — 質量が減少しました。',
      cannotVentMaterial: '{name} は排出できません。',
      insufficientCargo: '{name} が不足（必要 {need}、所持 {have}）。',
      confirmVent: '{name} {amount}t を大気へ排出しますか？資源は失われます。',
      ventedCargo: '{name} {amount}t を排出 — 搭載質量が減少しました。',
      corrosionWarn: '酸性腐食が進行 — 硫黄コーティングで上昇を抑えられます（ユニット選択→コーティング）。',
      corrosionSevere: '腐食深刻 — 電力消費・浮力低下が悪化。コーティングまたは S 維持を。',
      corrosionCritical: '腐食危険域 — 効率が大きく低下。今すぐ硫黄コーティングを。',
      sUpkeepActive: '硫黄維持中 — {amount}/tick を {count} 基の腐食抑制に消費。',
      h2LeakWarn: '水素漏れ — {rate}t/tick（腐食連動・コーティングで漏れ抑制）',
      electrolyzerRun: '水電解: H₂O分解 ×{amount} — H₂ 生成',
      contractComplete: '地球ミニ契約達成 — {title}：{reward}',
      buildLiftWarn: '注意: 配置後の正味浮力は {lift}t です — 建設は続行します',
    },
    contract: {
      rewardH2o: 'H₂O +{amount}',
      rewardFe: 'Fe +{amount}',
      rewardCredits: '₵ +{amount}',
      fourModules: {
        title: '大陸を広げる',
        progress: '契約開始から +{current}/{target} 基（合計 {total} 基目標）',
      },
      holdLift: {
        title: '正味浮力を維持',
        progress: '正味浮力 ≥ +{minLift} を {current}/{target} ティック（残り {remaining}）',
      },
      acidSplits: {
        title: '硫酸分解を重ねる',
        progress: '硫酸分解 {current}/{target} 回',
      },
    },
    isru: {
      status: {
        noIsru: 'ISRU未建設',
        noPower: '電力不足 — 停止中',
        acidReady: '硫酸分解可能',
        boschReady: 'ボッシュ可能',
        waitingAcid: 'H₂SO₄ 採取待ち',
        waitingH2: 'H₂不足（ボッシュ待ち）',
        electrolyzing: 'ボッシュ不可・電解中',
        idle: '待機中',
      },
      acidProgress: '次の H₂SO₄ まで: {pct}%（約 {eta} ティック）',
      acidProgressNoEta: '次の H₂SO₄ まで: {pct}%',
      intakeHintAffordable: 'ヒント: 大気インテークを建設して硫酸採取を加速できます',
      intakeHintFeShort: '鉄が不足 — 地球定期援助を待つか、地球市場で購入してください',
      intakeHintFeShortWithAid: '鉄が不足 — 次の地球援助まで {eta} ティック、または市場で購入',
      intakeHintCarbonShort: '炭素が不足 — ISRUボッシュで生成してください',
      intakeHintSulfurShort: '硫黄が不足 — ISRU硫酸分解で生成してください',
      fallbackElectrolysis: '副次: その間も CO₂ 電解は継続',
    },
  },
};

/** Apply static [data-i18n] text from the current locale. */
export function applyStaticI18n(root = document) {
  root.querySelectorAll('[data-i18n]').forEach((el) => {
    const key = el.getAttribute('data-i18n');
    if (key) el.textContent = t(key);
  });
  root.querySelectorAll('[data-i18n-aria]').forEach((el) => {
    const key = el.getAttribute('data-i18n-aria');
    if (key) el.setAttribute('aria-label', t(key));
  });
  root.querySelectorAll('[data-i18n-title]').forEach((el) => {
    const key = el.getAttribute('data-i18n-title');
    if (key) el.title = t(key);
  });
}
